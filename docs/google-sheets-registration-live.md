...
The previously considered private registration relay and associated server-restoration work are explicitly superseded. The active design sends the full registration only through the private Apps Script form POST, then returns a short-lived nonce-only status response after the official Sheet append. This preserves the static GitHub Pages deployment while providing a browser-confirmed result without storing event credentials in the public site.
